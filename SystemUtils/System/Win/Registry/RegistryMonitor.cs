...
namespace Util.System.Win.Registry {
  ...
  public class RegistryMonitor {
    ...
    private void ThreadLoop() {
      IntPtr registryKey;
      var result = UnsafeNativeMethods.RegOpenKeyEx(_registryHive, _registrySubName, 0,
      ...
      try {
        ...
        while (!_eventTerminate.WaitOne(0, true)) {
          result=UnsafeNativeMethods.RegNotifyChangeKeyValue(registryKey, true, _regFilter,
            eventNotify.SafeWaitHandle.DangerousGetHandle(), true);
          if (result!=0)
            throw new Win32Exception(result);
    ...
    private static class UnsafeNativeMethods {
      ...
      internal static extern int RegOpenKeyEx(IntPtr hKey, string subKey, uint options, int samDesired,
        out IntPtr phkResult);

      [DllImport("advapi32.dll", SetLastError = true)]
      internal static extern int RegNotifyChangeKeyValue(IntPtr hKey, bool bWatchSubtree,
    ...
    private static readonly Dictionary<RegistryHive, IntPtr> HivePtrMap = new Dictionary<RegistryHive, IntPtr> {
      ...
      {RegistryHive.DynData, HkeyDynData}
    };
    ...
    private void InitRegistryKey(string name) {
      var hive = name.SplitOnceAt('\\', out _registrySubName);
